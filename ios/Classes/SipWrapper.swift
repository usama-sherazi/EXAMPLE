...
class SipWrapper: NSObject, AbtoPhoneInterfaceObserver {
    ...
    private func initAbtoPhone() {
      if phone == nil {
        ...
        phone?.config().signalingTransport = PhoneSignalingTransport.tls
    ...
    private func receiveEvent(method: String!, arguments: Any?, result: @escaping FlutterResult) {
        ...
        case EVENT_UNREGISTER:
            ...
            phone?.unregister()
            phone?.deinitialize() //This now helps to not kill app when phone icon is pressed again
            break
    ...
    func onRegistered(_ accId: Int) {
        ...
        if let channel = channel {
            print("SipWrapper IOS: FlutterMethodChannel is initialized.")
            channel.invokeMethod(RESULT_REGISTERED, arguments: nil)
        } else {
            print("SipWrapper IOS: FlutterMethodChannel is nil!")
        }
    }
    ...
    private func buildConfigsMap() -> Dictionary<String, Any> {
        ...
        switch (cfg.signalingTransport) {
            ...
            default: signalingTransportKey = 3
        }
    ...
    private func applyConfigs(map: Dictionary<String, Any>) {
        ...
        case 1: signalingTransport = PhoneSignalingTransport.tls
        case 2: signalingTransport = PhoneSignalingTransport.tls
        case 3: signalingTransport = PhoneSignalingTransport.tls
        default: signalingTransport = PhoneSignalingTransport.tls
        }
        ...
        //confjgs
      //  configs.setTLSVerifyServer(false);
        configs.enableTLSVerifyServer = false
        phone?.finalizeConfiguration()
    }
}
